...
- `app/main.py` — FastAPI app, mounts static files, serves index
- `app/config.py` — NLLB model path, language map (code → NLLB code like `eng_Latn`), env vars
- `app/api/routes.py` — REST endpoints: `/api/translate` (POST), `/api/progress/{id}` (GET), `/api/download/{id}` (GET), `/api/languages` (GET), `/api/device-info` (GET)
...
- `app/translator/pipeline.py` — Orchestrator: splits lines into context batches via `ContextBatcher`, calls `NLLBEngine.translate_batch()` per batch, tracks progress with timing. `TranslationPipeline.translate()` is the main entry.
...
6. **GPU optimization** — For 1.3B model, small engine_batch (8) is faster than large (64) due to padding overhead.
...
### Add new language
Add to `NLLB_LANG_MAP` in `app/config.py` with correct NLLB code from [NLLB docs](https://github.com/facebookresearch/fairseq/tree/nllb).
